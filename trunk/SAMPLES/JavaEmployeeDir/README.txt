...
A2. Config the database in WebContent/WEB-INF/eo-config.xml
	 ...
	 <property name="java-source-local-dir" value="C:\JavaEmployeeDir\src"/> <!-- absolute path -->
...
A1. Config the database in WebContent/WEB-INF/eo-config.xml
     ...
     <db>C:\JavaEmployeeDir\db\derby_employeedir</db>		<!-- absolute path -->
	 <property name="java-source-local-dir" value="C:\JavaEmployeeDir\src"/> <!-- absolute path -->
...
C1. Click 'Metadata I/E' button on the navigation panel, browse the db/javaemployeedir-metadata.xml XML file and click 'Import' to import entities.

D. Run the web application using any web container (If you are using Derby, please close Athena Console first.).

E. Modify the remoting URL in FlexEmployeeDir/src/FlexEmployeeDir.mxml if you are using port other than the default 8080.
   'http://localhost:8080/JavaEmployeeDir/messagebroker/amf'
